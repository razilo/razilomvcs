 ...
 */
export default class RouteDataInventory extends Middleware
{
	constructor() {
		super();
	}
	 ...
	 */
	invoke(req, res, next) {
		console.log('check data structure for inventory, if not return invalid response...');

		if (false) {
			console.log('bad structure');
			res.status(400).json({'status': 'error', 'message': 'invalid data structure for inventory'});
		} else {
